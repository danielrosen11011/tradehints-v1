...
def google_news_rss(t: str, limit: int = 20) -> list[str]:
   """
   Lightweight fallback using Google News RSS.
   No API key needed. We just parse titles from the feed.
   """
   q = f"{t} stock"
   ...
   try:
       ...
       for item in root.findall(".//item")[:limit]:
           ...
           if title_el is None:
               continue
           h = (title_el.text or "").strip()
           if not h:
               continue
           if len(h) < 6:  # throw away junky 1–2 word titles
               continue
           heads.append(h)
       # de-dupe while preserving order
       return list(dict.fromkeys(heads))
...
def sp500_news_candidates(exclude: set[str], top_n: int = 3, days: int = 5) -> list[str]:
   ...
   for t in tickers:
       if t in exclude:
           continue
       heads = (
           fetch_headlines_yf(t)
           or finnhub_news(t, days, 10)
           or google_news_rss(t, 15)
       )
       if not heads:
           continue
       ns = news_signal(heads)
       es = earnings_signal(t, look=days)
       score = max(0.0, ns.get("score", 0)) * ns.get("conf", 0) * 0.6 \
             + max(0.0, es.get("score", 0)) * es.get("conf", 0) * 0.4
       if score > 0:
           ideas.append((t, score))
...
def analyze_ticker(t: str, owned: bool, cur_amt: float, horizon: int) -> dict:
   ...
   heads = (
       fetch_headlines_yf(t)
       or finnhub_news(t, 7, 15)
       or google_news_rss(t, 20)
   )
   senti = sentiment_score(heads)
   ns = news_signal(heads)
   es = earnings_signal(t)
   ind = technicals(df)
   fcast = forecast_price(df, horizon)
   ...
   return dict(
       ticker=t,
       owned=owned,
       cur_amt=round(cur_amt, 2),
       anomaly=anomaly,
       ind=ind,
       forecast=fcast,
       senti=senti,
       sent_desc="Positive" if senti > 0 else "Negative" if senti < 0 else "Neutral",
...
def main():
   ...
   if not trades:
       ...
       for tr in trades:
           print(f"{tr['side']:4} {tr['ticker']:6}  ${tr['dollars']:.2f}")

   print("\n================= PORTFOLIO META =================")
   print(f"Cash in:      ${meta.get('cash_in', 0):.2f}")
   print(f"Cash used:    ${meta.get('cash_used', 0):.2f}")
   print(f"Cash buffer:  ${meta.get('cash_buffer', 0):.2f}")
   print(f"Sectors:      {meta.get('sector_note', 'N/A')}")

   print("\n================ EXPLANATIONS ================")
   for s in summaries:
       print(f"\n--- {s['ticker']} ({s['action']})  ${s['dollars']:.2f} ---")
       print(s["explanation"])
